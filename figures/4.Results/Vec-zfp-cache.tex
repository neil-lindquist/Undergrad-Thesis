\begin{figure}
	\centering
	\begin{tikzpicture}% table
		\begin{axis}[axis lines = left,
					xlabel={Cache Size in Bytes},
					xticklabel={\(2^{\pgfmathprintnumber{\tick}}\)},
					ylabel={GFLOP/s Rating},
					ymax=17,
					ymin=0]
			\addplot[color=black,mark=*] table[x=cache,y=rating] {figures/4.Results/Vec-zfp-cache.csv};
			\draw (0, 15.3654) -- (100, 15.3654);
		\end{axis}
	\end{tikzpicture}
	\caption{Performance of 1d ZFP compression with 16 bits per value versus the size of the decoded values cache.  The horizontal line shows the performance of the baseline implementation.}
	\label{fig:results-zfp-cache}
\end{figure}